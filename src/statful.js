(function(window) {
  ...
  var statful = {
    ...
    initialize: function(clientConfig) {
      ...
      self.metricsData = function(
        ...
        return {
          ...
          type: type,
          value: value,
          tags: self.util.setTags(
            tags || {},
            self.config.tags,
            self.config[type].tags,
            self.config.app
          ),
          aggregations: self.util.setAggregations(
            aggregations,
            self.config.aggregations,
            self.config[type].aggregations
          ),
          aggregationFrequency: self.util.setAggregationFrequency(
            aggregationFrequency,
            self.config.aggregationFrequency,
            self.config[type].aggregationFrequency
          ),
    ...
    measureTimeUserTiming: function(measureName) {
      var time;
      var measure = statful.perf
        .getEntriesByName(measureName)
        .filter(function filterMeasures(entry) {
      ...
      try {
        ...
        if (measureName) {
          ...
          if (time) {
            ...
            this.util.addItemToQueue(
              "metrics",
              new this.metricsData(
                metricName,
                "timer",
                time,
                defaults.tags,
                defaults.aggregations,
                defaults.aggregationFrequency,
                defaults.namespace,
                defaults.sampleRate
              )
            );
    ...
    timer: function(metricName, metricValue, options) {
      try {
        ...
        if (metricName && metricValue >= 0) {
          ...
          logger.error(
            "Undefined metric name or invalid value to register as a timer"
          );
        }
      } catch (ex) {
        logger.error(ex);
